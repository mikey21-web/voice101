...
conn.on('ready', async () => {
  // Write SQL via base64
  await run(`echo ${b64} | base64 -d > /tmp/wf.sql`);
  console.log('SQL file written');
  
  const result = await run('docker exec -i demo-realestate-postgres-1 psql -U postgres lead_automation -f /tmp/wf.sql 2>&1');
