...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'workflow_definitions_tenant_id_fkey' AND table_name = 'workflow_definitions') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'workflow_versions_workflow_id_fkey' AND table_name = 'workflow_versions') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'workflow_triggers_version_id_fkey' AND table_name = 'workflow_triggers') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'workflow_steps_version_id_fkey' AND table_name = 'workflow_steps') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'workflow_edges_version_id_fkey' AND table_name = 'workflow_edges') THEN
...
conn.on('ready', async () => {
  // Write SQL to temp file on VPS
  await run(`cat > /tmp/wf_migration.sql << 'SQLEOF'
${SQL}
SQLEOF`);
  console.log('SQL written to VPS');
  
  // Execute via psql inside postgres container
  const result = await run('docker exec -i demo-realestate-postgres-1 psql -U postgres lead_automation -f /tmp/wf_migration.sql 2>&1');
