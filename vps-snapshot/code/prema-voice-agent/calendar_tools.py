...
def get_available_slots(date_str: str) -> list:
    ...
    try:
        ...
        for s in raw_slots:
            ...
            # Cross-platform: strftime("%-I") is Linux-only, "%#I" is Windows-only
            hour = dt.strftime("%I").lstrip("0") or "12"
            label = f"{hour}:{dt.strftime('%M %p')}"
            slots.append({
                "time":  s["time"],
                "label": label,   # e.g. "10:00 AM"
            })
...
async def create_booking(
    ...
    """
    Async booking function. Call with `await create_booking(...)`.
    Works correctly inside the agent's event loop.
    """
    return await async_create_booking(start_time, caller_name, caller_phone, notes)
