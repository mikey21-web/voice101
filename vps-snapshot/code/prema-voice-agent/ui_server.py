...
app = FastAPI(title="Priya AI Voice Agent Dashboard")
...
def read_config():
    ...
    return {
        "first_line": get_val("first_line", "FIRST_LINE",
            "Hello… Namaskaram! Nenu Priya maatlaadutunna… maa premium real estate team nundi. "
            "Hyderabad lo property kosam search chestunnara?"),
        "agent_instructions": get_val("agent_instructions", "AGENT_INSTRUCTIONS", ""),
        "min_endpointing_delay": float(get_val("min_endpointing_delay", "MIN_ENDPOINTING_DELAY", 0.8)),
        "llm_model": get_val("llm_model", "LLM_MODEL", "gpt-4o-mini"),
        "tts_voice": get_val("tts_voice", "TTS_VOICE", "anushka"),
        "tts_language": get_val("tts_language", "TTS_LANGUAGE", "te-IN"),
        "stt_language": get_val("stt_language", "STT_LANGUAGE", "te-IN"),
        "sales_transfer_number": get_val("sales_transfer_number", "DEFAULT_TRANSFER_NUMBER", ""),
...
# Config keys that downstream modules (agent.py, calendar_tools.py, notify.py, db.py)
# read via os.environ rather than config.json directly — must be synced before use.
_ENV_BRIDGE = {
    "sarvam_api_key": "SARVAM_API_KEY", "openai_api_key": "OPENAI_API_KEY",
    "cal_api_key": "CAL_API_KEY", "cal_event_type_id": "CAL_EVENT_TYPE_ID",
    "telegram_bot_token": "TELEGRAM_BOT_TOKEN", "telegram_chat_id": "TELEGRAM_CHAT_ID",
    "supabase_url": "SUPABASE_URL", "supabase_key": "SUPABASE_KEY",
    "vobiz_sip_domain": "VOBIZ_SIP_DOMAIN", "vobiz_username": "VOBIZ_USERNAME",
    "vobiz_password": "VOBIZ_PASSWORD", "vobiz_outbound_number": "VOBIZ_OUTBOUND_NUMBER",
    "sip_trunk_id": "SIP_TRUNK_ID", "sales_transfer_number": "DEFAULT_TRANSFER_NUMBER",
    "livekit_url": "LIVEKIT_URL", "livekit_api_key": "LIVEKIT_API_KEY",
    "livekit_api_secret": "LIVEKIT_API_SECRET",
}

def sync_env_from_config(config: dict = None):
    """Push dashboard-saved credentials into os.environ so other modules pick them up."""
    config = config or read_config()
    for cfg_key, env_key in _ENV_BRIDGE.items():
...
<nav id="sidebar">
  <div class="sidebar-brand">
    ...
    <div>
      ...
      <div class="brand-sub">Real Estate AI</div>
    </div>
  </div>
...
<div id="main">
  ...
  <div id="page-agent" class="page">
    ...
    <div class="section-card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="first_line" value="{config.get('first_line', '')}" placeholder="Hello… Namaskaram! Nenu Priya maatlaadutunna…">
    ...
    <div class="section-card">
      ...
      <div class="form-group" style="max-width:220px;">
        ...
        <input type="number" id="min_endpointing_delay" step="0.05" min="0.1" max="3.0" value="{config.get('min_endpointing_delay', 0.8)}">
        <div class="hint">Seconds the AI waits after silence before responding. Default: 0.8</div>
      </div>
    </div>
    <div class="section-card">
      <div class="section-title">Sales Transfer</div>
      <div class="form-group" style="max-width:320px;">
        <label>Transfer Number</label>
        <input type="text" id="sales_transfer_number" value="{config.get('sales_transfer_number', '')}" placeholder="+91...">
        <div class="hint">Number the agent transfers qualified leads to.</div>
      </div>
    </div>
    <div class="save-bar">
    ...
    <div class="section-card">
      ...
      <div class="form-group" style="max-width:360px;">
        ...
        <select id="llm_model">
          ...
          <option value="gpt-4o" {sel('llm_model','gpt-4o')}>gpt-4o — Smartest</option>
        </select>
      </div>
    </div>
    <div class="section-card">
      <div class="section-title">Voice Synthesis (Sarvam bulbul:v2)</div>
      <div class="form-row" style="max-width:720px;">
        <div class="form-group">
          <label>Speaker Voice</label>
          <select id="tts_voice">
            <option value="anushka" {sel('tts_voice','anushka')}>Anushka — Female, Natural Telugu</option>
        ...
        <div class="form-group">
          ...
          <select id="stt_language">
            <option value="te-IN" {sel('stt_language','te-IN')}>Telugu (te-IN)</option>
            <option value="hi-IN" {sel('stt_language','hi-IN')}>Hindi (hi-IN)</option>
            <option value="en-IN" {sel('stt_language','en-IN')}>English India (en-IN)</option>
            <option value="ta-IN" {sel('stt_language','ta-IN')}>Tamil (ta-IN)</option>
            <option value="kn-IN" {sel('stt_language','kn-IN')}>Kannada (kn-IN)</option>
            <option value="ml-IN" {sel('stt_language','ml-IN')}>Malayalam (ml-IN)</option>
            <option value="mr-IN" {sel('stt_language','mr-IN')}>Marathi (mr-IN)</option>
            <option value="gu-IN" {sel('stt_language','gu-IN')}>Gujarati (gu-IN)</option>
            <option value="bn-IN" {sel('stt_language','bn-IN')}>Bengali (bn-IN)</option>
          </select>
        </div>
      </div>
    </div>
  ...
  <div id="page-credentials" class="page">
    ...
    <div class="section-card">
      <div class="section-title">Vobiz SIP Trunk (Outbound Calling)</div>
      <div class="form-row">
        <div class="form-group"><label>SIP Domain</label><input type="text" id="vobiz_sip_domain" value="{config.get('vobiz_sip_domain', '')}" placeholder="xxx.sip.vobiz.ai"></div>
        <div class="form-group"><label>Outbound Number</label><input type="text" id="vobiz_outbound_number" value="{config.get('vobiz_outbound_number', '')}" placeholder="+91..."></div>
        <div class="form-group"><label>Username</label><input type="text" id="vobiz_username" value="{config.get('vobiz_username', '')}"></div>
        <div class="form-group"><label>Password</label><input type="password" id="vobiz_password" value="{config.get('vobiz_password', '')}"></div>
      </div>
    </div>
    <div class="section-card">
...
async function saveConfig(section) {{
  ...
  if (section === 'agent') {{
    Object.assign(payload, {{
      ...
      min_endpointing_delay: parseFloat(get('min_endpointing_delay')),
      sales_transfer_number: get('sales_transfer_number'),
    }});
    ...
    Object.assign(payload, {{
      ...
      vobiz_sip_domain: get('vobiz_sip_domain'), vobiz_outbound_number: get('vobiz_outbound_number'),
      vobiz_username: get('vobiz_username'), vobiz_password: get('vobiz_password'),
